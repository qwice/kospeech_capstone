...
def generate_character_labels(transcripts, labels_dest):
    ...
    label['id'] = label['id'][:40]
    label['char'] = label['char'][:40]
    label['freq'] = label['freq'][:40]

    label_df = pd.DataFrame(label)
    label_df.to_csv(os.path.join(labels_dest, "aihub_labels.csv"), encoding="utf-8", index=False, escapechar='/')
